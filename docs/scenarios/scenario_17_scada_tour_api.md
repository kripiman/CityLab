...
El estudiante explora la interfaz de supervisión industrial HMI P&ID que se ejecuta en el puerto HTTP `:8085`. Aprende a consultar el estado del proceso mediante las llamadas REST `/api/telemetry` e interactuar con la visualización web.
...
1. Ejecutar `attacker/attack_scada_tour.py`:
   ```bash
   python3 attacker/attack_scada_tour.py
   ```
