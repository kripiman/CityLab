...
- `network/citylab_gui.py` — native desktop dashboard app (Tkinter split-pane: HMI left / 2D Viz right).
...
- Always verify 0 orphan processes post-teardown (`citylab_procs` covers all emulators and DMZ daemons: `scada_server`, `hmi_server`, `viz_server`, `citylab_gui`, `modbus_proxy`, `siem_pipeline`, `flag_service`).
