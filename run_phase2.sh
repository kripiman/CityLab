#!/usr/bin/env bash
# run_phase2.sh — Punto de entrada para la Fase 2: Co-Simulación Multisectorial Interdependiente
set -euo pipefail

# Asegurar privilegios de root/sudo
if [ "$EUID" -ne 0 ]; then
  echo "[ERROR] Este script requiere privilegios de root. Ejecutar con: sudo ./run_phase2.sh"
  exit 1
fi
...
echo "[*] Iniciando broker HELICS (Fase 2: 4 federados)..."
HELICS_FED_COUNT=${HELICS_FED_COUNT:-6}
...
echo ""
echo "[+] Fase 2 en ejecución. Federados activos:"
