...
## Corpus Check
- 232 files · ~126,035 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2380 nodes · 3707 edges · 195 communities (188 shown, 7 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 224 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d454465b`
...
Cohesion: 0.05
Nodes (38): patch, Ejercita la lógica de producción poll_plcs_once() y verifica la transición real…, TestScenario21LossOfView, 6. Capa de Supervisión, DMZ y Servicios Centrales, 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Control, DPI Proxy e Historian, 🔀 3. Modbus DPI Proxy y Enrutamiento por Unit ID (`network/modbus_proxy.py`), 🔄 4. Alta Disponibilidad (HA) y Sincronización de Estado (`network/scada_ha.py`) (+30 more)
...
Cohesion: 0.07
Nodes (25): main(), Any, Explora la API REST y estado HMI SCADA vía HTTP o motor directo., ScadaTour, Verifica la exploración directa sobre el motor HMI., Verifica la consulta HTTP contra el servidor HMI real en puerto alto., Verifica la invocación CLI., TestScadaTour (+17 more)
...
### Community 15 - "NtpTimeSpoofingAttack"
Cohesion: 0.23
...
### Community 38 - "run_modbus_attack"
Cohesion: 0.17
...
Cohesion: 0.16
Nodes (15): 3. Topología de Red y Microsegmentación (IEC 62443), 10. Lo que este Plan NO Persigue, 1. Contexto y Justificación, 2. Objetivos Estratégicos y Métricas de Éxito, 3. Fase 0 — Cimientos: Manifiestos de Escenario y Semilla de Sesión, 5. Fase 2 — Contención: Egress, Namespaces y Jaula Probada, 6. Fase 3 — Ciclo de Vida por Cgroups y Secretos Fuera del Repo, 8. Fase 5 — Migración Masiva de Escenarios y Anti-Memorización (+7 more)

### Community 40 - "test_flag_service.py"
Cohesion: 0.08
Nodes (29): FlagServiceHandler, generate_flag_hmac(), load_scenario_manifest(), BaseHTTPRequestHandler, HTTPServer, Path, ThreadingMixIn, RateLimiter (+21 more)
...
### Community 43 - "Architecture — the big picture"
Cohesion: 0.12
Nodes (16): Architecture — the big picture, Golden rule — intentional vulnerabilities, Gotchas, What this is, 1. Visión General del Sistema y Filosofía de Diseño, 2. Estructura del Proyecto y Módulos de Código, 4. Capa Ciberfísica y Co-Simulación HELICS, 7. Pipeline de Seguridad: SIEM Central y Defensa Dinámica SDN (+8 more)
...
Cohesion: 0.07
Nodes (25): LiveSdnDefense, main(), Any, patch, TestScenario23Sdn, 5. Capa de Emulación de Dispositivos de Campo (OT), apply_circuit_breaker(), apply_sdn_flow_rules() (+17 more)
...
### Community 55 - "modbus_emulator.py"
Cohesion: 0.28
...
### Community 60 - "🚦 Federado 03 — Sector Transporte y Tráfico Urbano (`fed_transport.py`)"
Cohesion: 0.33
...
Cohesion: 0.19
Nodes (14): Verifica que el motor HMI registre alarma de LOSS_OF_VIEW y cambie a…, cmd_down(), Commands, 💾 6. Presupuesto de Recursos y Memoria RAM, 💾 6. Presupuesto de Recursos y Memoria RAM, 💾 6. Presupuesto de Recursos y Memoria RAM, 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Captura y Registro de Eventos (+6 more)
...
### Community 118 - "CityLab - Project Structure"
Cohesion: 0.40
Nodes (5): Architectural Patterns, Core Components, Directory Organization, CityLab - Project Structure, File Naming Conventions
...
### Community 167 - "ActuatorEmulator"
Cohesion: 0.28
Nodes (4): 📌 1. Visión General del Módulo, ActuatorEmulator, ModbusServerContext, Emula la lógica ST del PLC: TON arranque/parada y detección de fallo.

### Community 168 - "Iec61850Server"
Cohesion: 0.31
Nodes (6): Iec61850Server, Iec61850SvEncoder, is_multicast_addr(), main(), Codificador/Decodificador binario para Sampled Values (SV)., Servidor IED Subestación IEC 61850 con emisión GOOSE & SV y recepción de…
...
### Community 171 - "📘 Federado 01 — Sector Agua SWaT (Tratamiento y Distribución Multi-Etapa)"
Cohesion: 0.40
Nodes (5): ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico e Interdependencia Ciberfísica, 🗺️ 4. Mapa de Registros Modbus TCP (PLC Agua `10.0.3.10:502`), 📡 5. Interfaz de Co-Simulación HELICS, 📘 Federado 01 — Sector Agua SWaT (Tratamiento y Distribución Multi-Etapa)
...
Cohesion: 0.29
Nodes (6): Dnp3Server, main(), socket, Servidor TCP Outstation DNP3 para la subestación eléctrica., Inicia un Iec61850Server en puerto alto y asegura stop() en finally., running_iec61850_server()

### Community 174 - "🏥 Federado 04 — Sector Hospital Carga Crítica y Sistema ATS/UPS (`fed_hospital.p"
Cohesion: 0.40
Nodes (5): ⚙️ 2. Arquitectura de Código y Máquina de Estados de Energía, 🧮 3. Modelo Físico de Descarga de Baterías y Generador, 🗺️ 4. Mapa de Registros Modbus TCP (PLC Hospital `10.0.3.15:502`), 📡 5. Interfaz HELICS Pub-Sub, 🏥 Federado 04 — Sector Hospital Carga Crítica y Sistema ATS/UPS (`fed_hospital.p
...
### Community 186 - "⚡ Federado 02 — Sector Eléctrico y Subestación (GridLab-D & IEC 61850 / DNP3 SA)"
Cohesion: 0.40
Nodes (5): ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico e Inercia de Red, 🗺️ 4. Mapa de Protocolos (DNP3 SA `10.0.3.13` & IEC 61850 GOOSE `10.0.3.20`), 📡 5. Interfaz HELICS Pub-Sub, ⚡ Federado 02 — Sector Eléctrico y Subestación (GridLab-D & IEC 61850 / DNP3 SA)

### Community 187 - "main"
Cohesion: 0.33
Nodes (3): main(), Arranca el servidor. Bloqueante — llamar desde un hilo., Punto de entrada standalone del emulador OPC UA.

### Community 188 - "📌 1. Visión General del Módulo"
Cohesion: 0.22
Nodes (5): 📌 1. Visión General del Módulo, Calcula la pérdida de fricción en la tubería usando Hazen-Williams., Calcula la presión generada por la bomba según su curva TDH., Calcula el estado hidráulico de la red. Retorna (flow_m3_s, pressure_bar,…, Avanza el estado de la planta 2 etapas con motor EPANET. Retorna (t1_level_m3,…
...
## Knowledge Gaps
- **452 isolated node(s):** `PYTHONPATH`, `$schema`, `title`, `type`, `title` (+447 more)
...
- `hmi_server.py` (4× useful, score=3.992633652)
- `IndustrialHmiEngine` (3× useful, score=2.994551605)
- `viz_server.py` (3× useful, score=2.993670101)
- `CityVisualizerStateEngine` (2× useful, score=1.995588055)
- `fed_viz_bridge.py` (2× useful, score=1.994925843)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `HistorianTSDB` connect `HistorianTSDB` to `run_scenario.py`, `scada_server.py`, `test_flag_service.py`, `IndustrialHmiEngine`, `HistorianAntiForensicsAttack`, `PostIncidentRecovery`, `NtpTimeSpoofingAttack`, `._conn`, `SCADAAPIHandler`, `TestScadaHistorianHTTPEndpoints`, `TestHistorianTSDB`, `ConditionChecker`?**
  _High betweenness centrality (0.160) - this node is a cross-community bridge._
- **Why does `Architecture — the big picture` connect `Architecture — the big picture` to `📌 1. Visión General del Módulo`, `scada_server.py`, `EcsEvent`, `🎯 Roadmap — Medición del Aprendizaje y Contención del Laboratorio`, `ElectricalSubstationGrid`, `OtHoneypotServer`, `.decode`, `ActuatorEmulator`, `IndustrialHmiEngine`, `OpcUaServer`, `Dnp3OutstationState`, `RBACResolver`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `3. Requisitos Funcionales Específicos` connect `SafetyInstrumentedLogic` to `cmd_down`, `scada_server.py`, `SmartLightingSystem`, `EcsEvent`, `Architecture — the big picture`, `IndustrialHmiEngine`, `._conn`, `Dnp3OutstationState`?**
