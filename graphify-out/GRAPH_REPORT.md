# Graph Report - CityLab  (2026-09-14)

## Corpus Check
- 230 files · ~121,883 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2322 nodes · 3587 edges · 192 communities (185 shown, 7 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 212 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cf4e75ee`
...
- required
- RSS máximo del propio proceso vía `resource.getrusage` (stdlib, sin…
...
### Community 1 - "FlagServiceHandler"
Cohesion: 0.33
...
Cohesion: 0.14
Nodes (9): main(), ModbusDpiEngine, ModbusDpiProxyServer, socket, RateLimiter, Controlador de tasa de escrituras por IP de origen., Motor de Inspección Profunda de Paquetes (DPI) Modbus/TCP., Inspecciona la trama Modbus/TCP en Capa 7. Header Modbus TCP (MBAP): -… (+1 more)

### Community 6 - "ElectricalSubstationGrid"
Cohesion: 0.08
Nodes (23): 📌 1. Visión General del Módulo, create_federate(), HospitalPlant, main(), PowerState, Enum, helics_federate, helics_input (+15 more)
...
Cohesion: 0.13
Nodes (15): 📌 1. Visión General del Módulo, create_federate(), main(), Any, helics_federate, ModbusTcpClient, read_actuator_running(), ElecPlant (+7 more)

### Community 10 - "BacnetAttacker"
Cohesion: 0.07
Nodes (27): BacnetAttacker, main(), Any, Emulador de ataque BACnet/IP sobre UDP 47808., HoneypotTouch, main(), Any, main() (+19 more)
...
Cohesion: 0.05
Nodes (34): main(), Any, Explora la API REST y estado HMI SCADA vía HTTP o motor directo., ScadaTour, Verifica la exploración directa sobre el motor HMI., Verifica la consulta HTTP contra el servidor HMI real en puerto alto., Verifica la invocación CLI., TestScadaTour (+26 more)

### Community 14 - "ModbusReadOnly"
Cohesion: 0.31
...
### Community 15 - "🚦 Federado 03 — Sector Transporte y Tráfico Urbano (`fed_transport.py`)"
Cohesion: 0.33
Nodes (6): 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico de Congestión e Interdependencia, 🗺️ 4. Mapa de Registros Modbus TCP (PLC Transporte `10.0.3.14:502`), 📡 5. Interfaz HELICS Pub-Sub, 🚦 Federado 03 — Sector Transporte y Tráfico Urbano (`fed_transport.py`)
...
### Community 24 - "🛡️ Federado 05 — Sistema de Seguridad SIS / ESD (SIL-3) (`fed_sis.py`)"
Cohesion: 0.10
Nodes (19): 1. Introducción, ⚙️ 2. Arquitectura de Lógica Indesconectable SIL-3, 🧮 3. Envolvente de Seguridad Física (Safety Limits), 📡 4. Interfaz HELICS Pub-Sub, 💾 5. Presupuesto de Recursos y Memoria RAM, 🛡️ Federado 05 — Sistema de Seguridad SIS / ESD (SIL-3) (`fed_sis.py`), 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Captura y Registro de Eventos (+11 more)

### Community 25 - "SafetyInstrumentedLogic"
Cohesion: 0.11
Nodes (12): 📌 1. Visión General del Módulo, create_federate(), main(), Any, helics_sim/fed_sis.py — Safety Instrumented System (SIS / ESD Independiente)…, Lógica de interlocks SIL-3 independiente., Evalúa los interlocks de seguridad física. Retorna (must_trip, reason)., Aplica la regla de anulación SIS sobre comandos BPCS. Retorna (allowed_cmd,… (+4 more)
...
### Community 30 - "running_modbus_server"
Cohesion: 0.18
Nodes (7): El entrypoint CLI main() ejecuta el escaneo activo con flags sin errores., Verifica ejecución de la CLI., El entrypoint CLI main() ejecuta el vector replay contra el puerto de test sin…, El entrypoint CLI main() ejecuta la manipulación Triton sobre el puerto de test…, ModbusServerContext, Inicia un ModbusTcpServer en puerto alto y asegura shutdown/server_close en…, running_modbus_server()

### Community 31 - "KerberosServerThread"
Cohesion: 0.13
Nodes (13): DomainControllerEmulator, KerberosServerThread, LdapServerThread, main(), socket, Escuchador SMB v2/v3 en puerto 445., Orquestador completo del controlador de dominio h_dc., Escuchador LDAP en puerto 389. (+5 more)
...
Cohesion: 0.18
Nodes (11): EcsEvent, forward_alert_to_visualizer(), forward_event_to_central_siem(), Any, BaseHTTPRequestHandler, Aplica reglas de correlación SOC sobre los eventos ingresados., Ingiere y normaliza un registro de log Zeek (conn.log, notice.log, modbus.log)., Ingiere y normaliza un registro de alerta Suricata Eve JSON (eve.json). (+3 more)

### Community 38 - "Especificación de Requisitos de Software (ERS)"
Cohesion: 0.17
Nodes (12): Gotchas, 2. Descripción General, 4. Requisitos No Funcionales (RNF), Especificación de Requisitos de Software (ERS), Proyecto: Cyber Range Ciberfísico Multisectorial (CityLab), 🎯 DECISIONES DE DISEÑO PEDAGÓGICO CTF (HALLAZGOS INTENCIONALES), ✅ REPORTE DE CIERRE DE AUDITORÍA — CityLab Cyber Range, 📈 EVOLUCIÓN DEL GRAFO DE CONOCIMIENTO (+4 more)

### Community 39 - "cmd_down"
Cohesion: 0.23
Nodes (12): cmd_down(), 10. Lo que este Plan NO Persigue, 1. Contexto y Justificación, 2. Objetivos Estratégicos y Métricas de Éxito, 3. Fase 0 — Cimientos: Manifiestos de Escenario y Semilla de Sesión, 4. Fase 1 — Servicio de Flags Verificables (Scoring Server-Side), 5. Fase 2 — Contención: Egress, Namespaces y Jaula Probada, 6. Fase 3 — Ciclo de Vida por Cgroups y Secretos Fuera del Repo (+4 more)

### Community 40 - "test_flag_service.py"
Cohesion: 0.11
Nodes (17): fixture, RateLimiter, Limitador de tasa de peticiones deslizante por clave., Tests unitarios y de integración para network/flag_service.py (Fase 1 - Roadmap), Verifica la evaluación del oráculo de estado de sector SCADA., Verifica el oráculo de estado del visualizador 2D para sectores extendidos…, Inicia un ThreadedFlagServer en un puerto efímero de test., Prueba los endpoints HTTP /health, /api/flag/mint, /api/flag/submit y… (+9 more)
...
Cohesion: 0.14
Nodes (15): Code Quality Standards Analysis, Development Workflow Guidelines, CityLab - Development Guidelines, Practices Followed Throughout Codebase, Semantic Patterns Overview, Structural Conventions, Textual Standards, Cadena de Sabotaje Eléctrico y Cascada Ciberfísica (+7 more)

### Community 43 - "🛡️ Arquitectura del Cyber Range CityLab (IEC 62443 / Co-Simulación Ciberfísica)"
Cohesion: 0.17
Nodes (12): Architectural Patterns, Core Components, Directory Organization, CityLab - Project Structure, File Naming Conventions, 1. Visión General del Sistema y Filosofía de Diseño, 2. Estructura del Proyecto y Módulos de Código, 3. Topología de Red y Microsegmentación (IEC 62443) (+4 more)

### Community 44 - "Dnp3BreakerAttack"
Cohesion: 0.18
...
Cohesion: 0.15
Nodes (11): HistorianAntiForensicsAttack, main(), Any, Verifica que el ataque purgue físicamente los registros de telemetría de la…, Verifica ejecución CLI con DB temporal., TestAntiForensicsAttack, HistorianTSDB, Path (+3 more)
...
Cohesion: 0.06
Nodes (31): LiveSdnDefense, main(), Any, patch, TestScenario23Sdn, 7. Pipeline de Seguridad: SIEM Central y Defensa Dinámica SDN, 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Reenvío Asíncrono y Detección SIEM (+23 more)
...
### Community 55 - "Development Workflow"
Cohesion: 0.25
...
### Community 60 - "3. Requisitos Funcionales Específicos"
Cohesion: 0.18
Nodes (14): 6. Capa de Supervisión, DMZ y Servicios Centrales, 3. Requisitos Funcionales Específicos, 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Control, DPI Proxy e Historian, 🔀 3. Modbus DPI Proxy y Enrutamiento por Unit ID (`network/modbus_proxy.py`), 🔄 4. Alta Disponibilidad (HA) y Sincronización de Estado (`network/scada_ha.py`), 🔐 5. Control de Acceso por Roles (RBAC Bearer Estático & Toggle `STRICT_AUTH`), 🌐 6. Endpoints REST API de Infraestructura SCADA / HMI / Viz (`:8080`, `:8085`,  (+6 more)
...
### Community 79 - "TwoStageWaterPlant"
Cohesion: 0.16
Nodes (9): TestPhysicsEngine, EpanetHydraulicSolver, PipeConfig, PumpConfig, Solver hidráulico de red de distribución de agua (Modelo didáctico Hazen-…, physical/water package, physical/water/plant_water.py — Modelo físico de tratamiento de agua en 2…, Regla de disparo de protección de planta: - T1 desborde (>95%) o seco (<0.5m³)… (+1 more)
...
Cohesion: 0.17
Nodes (15): connect(), do_fault(), do_start_stop_blast(), main(), ModbusTcpClient, Ejecuta ataque Modbus/TCP contra PLC objetivo vía socket real o fallback…, read_coils(), run_modbus_attack() (+7 more)

### Community 85 - "BlindRandomizedEnv"
Cohesion: 0.23
Nodes (7): BlindRandomizedEnv, main(), Any, Path, Escribe los parametros aleatorizados a disco para que topology.py/fed_sis.py…, TestScenario28Blind, 8. Fase 5 — Migración Masiva de Escenarios y Anti-Memorización

### Community 86 - "RansomwareTabletop"
Cohesion: 0.43
Nodes (4): main(), Any, RansomwareTabletop, TestScenario25Tabletop

### Community 87 - "RansomwareOtImpactAttack"
Cohesion: 0.43
...
### Community 100 - "RSS máximo del propio proceso vía `resource.getrusage` (stdlib, sin…"
Cohesion: 0.13
Nodes (15): ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico e Inercia de Red, 🗺️ 4. Mapa de Protocolos (DNP3 SA `10.0.3.13` & IEC 61850 GOOSE `10.0.3.20`), 📡 5. Interfaz HELICS Pub-Sub, 💾 6. Presupuesto de Recursos y Memoria RAM, ⚡ Federado 02 — Sector Eléctrico y Subestación (GridLab-D & IEC 61850 / DNP3 SA), 💾 6. Presupuesto de Recursos y Memoria RAM, ⚙️ 2. Arquitectura de Código y Máquina de Estados de Energía (+7 more)
...
### Community 103 - "flag_service.py"
Cohesion: 0.18
Nodes (8): HistorianTSDB, HTTPServer, ThreadingMixIn, Servidor HTTP multihilo para el servicio de flags y scoring., Inicia el demonio de Flag Service., run_flag_service(), ThreadedFlagServer, Path
...
Cohesion: 0.09
Nodes (15): 5. Capa de Emulación de Dispositivos de Campo (OT), ModbusTcpServer, ActuatorEmulator, BacnetListener, build_server(), main(), NtcipListener, ModbusServerContext (+7 more)
...
### Community 167 - "NtpTimeSpoofingAttack"
Cohesion: 0.23
...
### Community 168 - "scada_server.py"
Cohesion: 0.24
Nodes (8): main(), poll_plcs(), poll_plcs_once(), Any, Hilo de fondo que consulta periódicamente los PLCs OT., Ejecuta una ronda individual de consulta a los PLCs OT (directo o vía DPI…, patch, Verifica que USE_MODBUS_PROXY=1 dirija las peticiones al proxy con Unit ID…

### Community 169 - "Iec61850Server"
Cohesion: 0.06
Nodes (28): is_multicast_addr(), main(), Construye y transmite un paquete GOOSE malicioso de disparo de interruptor., spoof_goose_trip(), Verifica la codificación y decodificación binaria del PDU GOOSE., Ataque real vía socket UDP provoca mutación de estado observable en el IED., Paquetes UDP malformados no deben mutar el estado del interruptor., El entrypoint CLI main() ejecuta ráfagas de spoofing sobre el puerto de test… (+20 more)

### Community 170 - "schema.json"
Cohesion: 0.40
Nodes (4): additionalProperties, $schema, title, type

### Community 171 - "📌 1. Visión General del Módulo"
Cohesion: 0.22
Nodes (8): 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico e Interdependencia Ciberfísica, 🗺️ 4. Mapa de Registros Modbus TCP (PLC Agua `10.0.3.10:502`), 📡 5. Interfaz de Co-Simulación HELICS, 💾 6. Presupuesto de Recursos y Memoria RAM, 📘 Federado 01 — Sector Agua SWaT (Tratamiento y Distribución Multi-Etapa), Avanza el estado de la planta 2 etapas con motor EPANET. Retorna (t1_level_m3,…
...
### Community 173 - "_emulator_harness.py"
Cohesion: 0.39
Nodes (4): Dnp3Server, main(), socket, Servidor TCP Outstation DNP3 para la subestación eléctrica.

### Community 174 - "generate_flag_hmac"
Cohesion: 0.40
Nodes (6): generate_flag_hmac(), Genera una flag dinámica e inmemorizable: FLAG_<id>{HMAC_SHA256(seed,…, Verifica si la flag enviada coincide exactamente con la calculada para la…, verify_flag_hmac(), Verifica que el HMAC sea determinista con la misma semilla y distinto con otra., test_hmac_determinism_and_uniqueness()
...
### Community 191 - "ThreadedMockScadaServer"
Cohesion: 0.50
...
## Knowledge Gaps
- **449 isolated node(s):** `$schema`, `title`, `type`, `title`, `objectives` (+444 more)
...
- **Why does `HistorianTSDB` connect `HistorianTSDB` to `run_scenario.py`, `historian.py`, `NtpTimeSpoofingAttack`, `scada_server.py`, `test_flag_service.py`, `._conn`, `IndustrialHmiEngine`, `PostIncidentRecovery`, `SCADAAPIHandler`, `3. Requisitos Funcionales Específicos`, `TestHistorianTSDB`?**
  _High betweenness centrality (0.098) - this node is a cross-community bridge._
- **Why does `Architecture — the big picture` connect `IndustrialHmiEngine` to `EcsEvent`, `ElectricalSubstationGrid`, `cmd_down`, `OtHoneypotServer`, `Iec61850Server`, `BacnetListener`, `OpcUaServer`, `Dnp3OutstationState`, `RBACResolver`, `3. Requisitos Funcionales Específicos`, `KerberosServerThread`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `3. Requisitos Funcionales Específicos` connect `3. Requisitos Funcionales Específicos` to `SmartLightingSystem`, `EcsEvent`, `RSS máximo del propio proceso vía `resource.getrusage` (stdlib, sin…`, `cmd_down`, `Especificación de Requisitos de Software (ERS)`, `IndustrialHmiEngine`, `Dnp3OutstationState`, `🛡️ Federado 05 — Sistema de Seguridad SIS / ESD (SIL-3) (`fed_sis.py`)`, `SafetyInstrumentedLogic`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Are the 12 inferred relationships involving `HistorianTSDB` (e.g. with `HistorianAntiForensicsAttack` and `NtpTimeSpoofingAttack`) actually correct?**
  _`HistorianTSDB` has 12 INFERRED edges - model-reasoned connections that need verification._
