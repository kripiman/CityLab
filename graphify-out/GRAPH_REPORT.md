# Graph Report - CityLab  (2026-09-03)

## Corpus Check
- 221 files · ~111,265 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2219 nodes · 3461 edges · 182 communities (176 shown, 6 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 217 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7b67ed62`
...
Cohesion: 0.07
Nodes (36): FlagServiceHandler, generate_flag_hmac(), load_scenario_manifest(), BaseHTTPRequestHandler, HTTPServer, ThreadingMixIn, RateLimiter, Carga y parsea el archivo YAML del manifiesto de escenario. (+28 more)
...
### Community 3 - "Dnp3OutstationState"
Cohesion: 0.08
Nodes (21): LiveSdnDefense, main(), Any, patch, TestScenario23Sdn, apply_circuit_breaker(), apply_sdn_flow_rules(), main() (+13 more)
...
Cohesion: 0.12
Nodes (11): main(), ModbusDpiEngine, ModbusDpiProxyServer, socket, RateLimiter, Controlador de tasa de escrituras por IP de origen., Motor de Inspección Profunda de Paquetes (DPI) Modbus/TCP., Inspecciona la trama Modbus/TCP en Capa 7. Header Modbus TCP (MBAP): -… (+3 more)
...
Cohesion: 0.18
Nodes (10): BacnetAttacker, main(), Any, Emulador de ataque BACnet/IP sobre UDP 47808., Verifica que el ataque BACnet mute el estado de alarma del listener real a…, Verifica fallback ante puerto BACnet cerrado., TestBacnetNtcipAttacks, Any (+2 more)
...
### Community 14 - "Dnp3MasterClient"
Cohesion: 0.06
Nodes (33): Dnp3BreakerAttack, main(), Any, Emulador de ataque DNP3 CROB sobre TCP 20000., Verifica que el ataque DNP3 CROB TRIP abra el disyuntor real en el servidor…, Verifica que el ataque DNP3 CROB CLOSE cierre el disyuntor real., Verifica que si el outstation DNP3 está inalcanzable caiga en TABLETOP_FALLBACK., Verifica invocación CLI. (+25 more)

### Community 15 - "cmd_down"
Cohesion: 0.09
Nodes (26): cmd_down(), 2. Descripción General, 3. Requisitos Funcionales Específicos, 4. Requisitos No Funcionales (RNF), Especificación de Requisitos de Software (ERS), Proyecto: Cyber Range Ciberfísico Multisectorial (CityLab), ⚙️ 2. Arquitectura de Código y Flujo de Trabajo, 🧮 3. Modelo Físico e Inercia de Red (+18 more)
...
Cohesion: 0.10
Nodes (19): Build Systems and Development Tools, Core Dependencies, Development Workflow, CityLab - Technology Stack, Platform Requirements, Programming Languages and Versions, Runtime Environment, Any (+11 more)
...
### Community 30 - "OpcUaClient"
Cohesion: 0.13
Nodes (10): Dnp3Server, main(), Servidor TCP Outstation DNP3 para la subestación eléctrica., OpcUaClient, Cliente OPC UA TCP mínimo para pruebas de integración. Replica exactamente el…, Lee el valor de un nodo OPC UA., Escribe un valor numérico a un nodo OPC UA sobre la red., Consulta los endpoints disponibles (GetEndpoints). (+2 more)

### Community 31 - "OtActiveScan"
Cohesion: 0.21
Nodes (9): main(), OtActiveScan, Any, Escáner activo de servicios y puertos OT., Realiza un probe TCP connect a un endpoint host:port con timeout corto., Ejecuta el escaneo activo TCP sobre la lista de objetivos dada o los defaults., Verifica que el escáner detecte empíricamente puertos abiertos y cerrados…, Si no hay objetivos alcanzables en localhost, aplica fallback TABLETOP_FALLBACK… (+1 more)
...
Cohesion: 0.16
Nodes (13): 3. Topología de Red y Microsegmentación (IEC 62443), 10. Lo que este Plan NO Persigue, 2. Objetivos Estratégicos y Métricas de Éxito, 3. Fase 0 — Cimientos: Manifiestos de Escenario y Semilla de Sesión, 4. Fase 1 — Servicio de Flags Verificables (Scoring Server-Side), 5. Fase 2 — Contención: Egress, Namespaces y Jaula Probada, 6. Fase 3 — Ciclo de Vida por Cgroups y Secretos Fuera del Repo, 8. Fase 5 — Migración Masiva de Escenarios y Anti-Memorización (+5 more)

### Community 40 - "Architecture — the big picture"
Cohesion: 0.16
Nodes (16): Architecture — the big picture, 6. Capa de Supervisión, DMZ y Servicios Centrales, 7. Pipeline de Seguridad: SIEM Central y Defensa Dinámica SDN, 📌 1. Visión General del Módulo, ⚙️ 2. Arquitectura de Control, DPI Proxy e Historian, 🔀 3. Modbus DPI Proxy y Enrutamiento por Unit ID (`network/modbus_proxy.py`), 🔄 4. Alta Disponibilidad (HA) y Sincronización de Estado (`network/scada_ha.py`), 🔐 5. Control de Acceso por Roles (RBAC Bearer Estático & Toggle `STRICT_AUTH`) (+8 more)
...
Cohesion: 0.22
Nodes (7): KerberoastAttack, main(), Any, Verifica si el servicio KDC Kerberos está respondiendo en la red., Verifica que el ataque solicite y reciba un ticket TGS vía socket real TCP al…, Verifica que si el KDC está inaccesible caiga en TABLETOP_FALLBACK sin romper., TestKerberoastAttack
...
### Community 55 - "NtcipAttacker"
Cohesion: 0.29
Nodes (5): main(), NtcipAttacker, Any, Emulador de ataque NTCIP 1202 sobre TCP 161., Verifica fallback ante puerto NTCIP cerrado.
...
### Community 78 - "TestSentinelSanitization"
Cohesion: 0.18
Nodes (6): Pruebas de lógica pura de sanitización según especificación…, Valores no inicializados (-9223372036854775808) no deben activar disparo…, Un valor explícito 1 debe activar el disparo independientemente de otros…, Verifica la lógica de combinación multi-sector en fed_gridmock., Valores double pre-publicación (-9.99e48) deben normalizarse a 0.0 kW., TestSentinelSanitization
...
Cohesion: 0.15
Nodes (10): patch, Ejercita la lógica de producción poll_plcs_once() y verifica la transición real…, main(), poll_plcs(), poll_plcs_once(), Any, BaseHTTPRequestHandler, Hilo de fondo que consulta periódicamente los PLCs OT. (+2 more)
...
### Community 100 - "✅ REPORTE DE CIERRE DE AUDITORÍA — CityLab Cyber Range"
Cohesion: 0.29
...
### Community 167 - "Commands"
Cohesion: 0.29
Nodes (5): Verifica que el motor HMI registre alarma de LOSS_OF_VIEW y cambie a…, Commands, Golden rule — intentional vulnerabilities, Gotchas, What this is
...
### Community 170 - "🏥 Federado 04 — Sector Hospital Carga Crítica y Sistema ATS/UPS (`fed_hospital.p"
Cohesion: 0.33
...
### Community 174 - "running_ntcip_server"
Cohesion: 0.40
Nodes (3): Verifica que el ataque NTCIP 1202 altere la fase del semáforo a FLASHING_YELLOW., Inicia un NtcipListener en puerto alto y asegura stop() en finally., running_ntcip_server()

### Community 175 - "run_modbus_attack"
Cohesion: 0.45
...
### Community 176 - "TestGooseSpoofingAttack"
Cohesion: 0.18
...
### Community 177 - "attack_grid_heatwave_attribution.py"
Cohesion: 0.33
...
## Knowledge Gaps
- **419 isolated node(s):** `PYTHONPATH`, `$schema`, `title`, `type`, `title` (+414 more)
...
- **Why does `HistorianTSDB` connect `HistorianTSDB` to `test_flag_service.py`, `HmiRequestHandler`, `🎯 Roadmap — Medición del Aprendizaje y Contención del Laboratorio`, `._conn`, `IndustrialHmiEngine`, `PostIncidentRecovery`, `.__init__`, `SCADAAPIHandler`, `TestScadaHistorianHTTPEndpoints`, `TestHistorianTSDB`, `ConditionChecker`?**
  _High betweenness centrality (0.121) - this node is a cross-community bridge._
- **Why does `Architecture — the big picture` connect `Architecture — the big picture` to `.decode`, `Dnp3OutstationState`, `EcsEvent`, `ElectricalSubstationGrid`, `🎯 Roadmap — Medición del Aprendizaje y Contención del Laboratorio`, `Commands`, `OtHoneypotServer`, `IndustrialHmiEngine`, `Dnp3MasterClient`, `build_server`, `_emulator_harness.py`, `RBACResolver`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `SiemCorrelationEngine` connect `SiemCorrelationEngine` to `HistorianTSDB`, `ModbusDpiEngine`, `EcsEvent`, `BacnetListener`, `HoneypotTouch`, `.export_elk_json`, `build_server`, `PurpleTeamMttd`, `siem_pipeline.py`, `NtcipListener`, `OtHoneypotServer`?**
